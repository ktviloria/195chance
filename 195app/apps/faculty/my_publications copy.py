...
layout= html.Div( 
    [ 
        dbc.Card(
            [
                dbc.CardHeader(html.H2("My Publications")),
                dbc.CardBody(
                    [
                        dbc.Row(
                            [
                                dbc.Col(
                                    ...
                                    dbc.DropdownMenu(
                                    children=[
                                        dbc.DropdownMenuItem("Authorships", href="form_authorships?mode=add"),
                                        ...
                                        dbc.DropdownMenuItem("Others", href="form_others?mode=add"),
                                    ],
                                    label='Add Publication', color='danger'),
                                    ]
                                ),
                                dbc.Col(
                                    html.Div(
                        ...
                        dbc.Row(
                            [
                                ...
                                dbc.Col(
                                    html.Div(
                                        dcc.Dropdown(id='mypub_date_filter', clearable=True, searchable=True, placeholder="LOWER YYYY"),
                                ...
                                dbc.Col(
                                    html.Div(
                                        dcc.Dropdown(id='mypub_date_filter_upper', clearable=True, searchable=True, placeholder="UPPER YYYY"),
                                        className="dash-bootstrap" 
                                    ),
                                    style={'min-width': '8%', 'padding-left': '0.5rem', 'padding-right': '0.5rem'}
                                ),
                                #search filter
                                dbc.Col(
                                    dbc.Input(type="text", id="mypub_filter", placeholder="Search"),
...
def mypub_loadpublist(pathname, tab, searchterm, datefilter, datefilter_u,  currentuserid):
    if pathname == '/my_publications':      
        if tab == 'tab_a':
            ...
            cols_a = ['Year','id', 'Faculty Involved', 'Publication Criteria', 'Title', 'All Authors', 'Involvement', 'Date', 'Publication', 'Publisher', 'DOI','ISXN', 'Scopus', 'Last Updated', 'Last Modified By']
            ...
            cols_p = ['Year', 'id', 'Faculty Involved', 'Publication Criteria', 'Title', 'All Authors',  'Start Date', 'End Date', 'Conference',
            ...
            # if searchterm:
            #     #OR (tag_short_title %s)
            # #OR (p_start_date ILIKE %s) OR (p_end_date ILIKE %s)
            #     sql_p += """ AND ((faculty_fn || ' ' || faculty_ln) ILIKE %s) OR (pub_title ILIKE %s) 
            #         OR (p_authors ILIKE %s) OR (p_year ILIKE %s) OR (p_conf ILIKE %s)
            #         OR (p_loc ILIKE %s) OR (p_add_info ILIKE %s)"""
            #     values_p += [f"%{searchterm}%", f"%{searchterm}%", f"%{searchterm}%", f"%{searchterm}%", 
            ...
            # if pub_p.shape[0]: 
            #     buttons_p = [] 
            #     for currentuserid in pub_p['userID']: 
            #         buttons_p += [ 
            #             html.Div( 
            #                 dbc.Button('Edit/Delete', href=f"/form_publications?mode=edit&id={currentuserid}", size='sm', color='secondary', ), 
            #                 style={'text-align': 'center'} 
            #             ) 
            #         ] 
            #     pub_p['Action'] = buttons_p
            # pub_p.drop(['userID'],axis=1,inplace=True)
            # table_p = dbc.Table.from_dataframe(pub_p, striped=True, bordered=True, hover=True, size='sm') 
            # return [table_p] 
            ...
            cols_r = ['Year', 'id', 'Faculty Involved', 'Publication Criteria', 'Title', 'Role', 'Timeframe', 'Start Date', 'End Date', 'Funding Organization',
            ...
            cols_o = ['Year', 'id', 'userID', 'Faculty Involved', 'Publication Criteria', 'Title', 'Date', "Other Info", "Last Updated", "Last Modified By"] 
...
def download_mypub(tab, n_clicks,searchterm, datefilter, datefilter_u,  currentuserid):
    ...
    if n_clicks==1:
        ...
        if tab == 'tab_a':
            ...
            cols_a_down = ['Year','id', 'Faculty Involved', 'Publication Criteria', 'Title', 'All Authors', 'Involvement', 'Date', 'Publication', 'Publisher', 'DOI','ISXN', 'Scopus', 'Last Updated', 'Last Modified By']
            ...
            sql_a_down += """ORDER BY authorships.a_year DESC"""         
            pub_a_down = db.querydatafromdatabase(sql_a_down, values_a_down, cols_a_down)
            
            download_table_a = dcc.send_data_frame(pub_a_down.to_csv, "myauthorships_download.csv")
            ...
            cols_p_down = ['Year', 'id', 'Faculty Involved', 'Publication Criteria', 'Title', 'Presenter/s',  'Start Date', 'End Date', 'Conference',
                      'Location', 'Other Info', 'Last Updated', 'Last Modified By']
            ...
            sql_p_down += """ORDER BY presentations.p_year DESC"""
            ...
            cols_r_down = ['Year', 'id', 'Faculty Involved', 'Publication Criteria', 'Title', 'Role', 'Timeframe', 'Start Date', 'End Date', 'Funding Organization', 
                      'Last Updated', 'Last Modified By'] 
            ...
            sql_r_down += """ORDER BY projects.r_year DESC"""
            ...
            cols_o_down = ['Year', 'id', 'Faculty Involved', 'Publication Criteria', 'Title', 'Date', "Other Info", "Last Updated", "Last Modified By"]
            ...
            sql_o_down += """ORDER BY others.o_year DESC"""
            pub_o_down = db.querydatafromdatabase(sql_o_down, values_o_down, cols_o_down)

            download_table_o = dcc.send_data_frame(pub_o_down.to_csv, "myothers_download.csv")
