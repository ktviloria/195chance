            ...
            [
                dbc.Col(
                    [
                        dbc.Card(
                            [
                            dbc.CardHeader ([html.H5("My Academic Research and Activity Summary:", style={'fontWeight':'bold'})]),
                            dbc.CardBody(
                                [
...
def facdetails_load(pathname, datefilter, search, currentuserid): 
    if pathname == '/my_profile': 
        ...
        sql2="""SELECT distinct (pub_type) as label,  year, count (pub_id) as value from 
                (
                    select authorships_users.pub_id as pub_id, authorships.a_year as year,
						case
                    ...
                    select presentations_users.pub_id as pub_id, presentations.p_year as year, 
                        case
                    ...
                    select projects_users.pub_id as pub_id, projects.r_year as year,
                        case
                    ...
                    select others.pub_id as pub_id, others.o_year as year,
                        case
                            when tag_id <=13 then 'Other Academic Merit'
                        end as "pub_type"
                    from others_users
        ...
        cols2 = ['Type', 'Year', 'No. of Academic Research and Activity']

        #sql2 += """order by year"""
        #values2 += [] 
        pub2 = db.querydatafromdatabase(sql2, values2, cols2)
        pubsumm_graph1 = px.line(pub2, x='Year', y='No. of Academic Research and Activity', color='Type', title = 'My Academic Research and Activity Through the Years')
        
        pubsumm_graph1 = pubsumm_graph1.update_xaxes(categoryorder='category ascending')
        ...
        sql3="""SELECT
            ...
            from
				...
				select
                    presentations_users.pub_id AS pub_id,
                    tag_short_title as tag,
				...
				select
                    projects_users.pub_id AS pub_id,
                    tag_short_title as tag,
                    cast(r_year as int) as year,
                    publications.tag_id,
                    pub_delete_ind
                from projects_users
                LEFT OUTER JOIN publications ON projects_users.pub_id = publications.pub_id
				...
				left outer join tags on publications.tag_id = tags.tag_id
                INNER JOIN faculty on projects_users.user_id = faculty.user_id
                where pub_delete_ind = false  AND faculty.user_id = %s
				 
				union all
				
				select
                    others_users.pub_id AS pub_id,
                    tag_short_title as tag,
                    cast(o_year as int) as year,
                    publications.tag_id,
                    pub_delete_ind
                from others_users
                LEFT OUTER JOIN publications ON others_users.pub_id = publications.pub_id
                ...
                left outer join tags on publications.tag_id = tags.tag_id
                INNER JOIN faculty on others_users.user_id = faculty.user_id
                where pub_delete_ind = false AND faculty.user_id = %s
                )
            as tag_summ
        ...
        cols3 = ['Criteria', 'Year', 'No. of Academic Research and Activity', 'Type']

        if datefilter:
            sql3 += """HAVING (CAST(year AS varchar) ILIKE %s)"""
            values3 += [f"%{datefilter}%"] 
            pub3 = db.querydatafromdatabase(sql3, values3, cols3)
            pubsumm_graph2 = px.bar(pub3, x='Type', y='No. of Academic Research and Activity', color='Criteria', title = 'My Academic Research and Activity per Type for Selected Year')
        else:
            sql3 += """order by year"""
            values3 += [] 
            pub3 = db.querydatafromdatabase(sql3, values3, cols3)
            pubsumm_graph2 = px.bar(pub3, x='Year', y='No. of Academic Research and Activity', color='Type', hover_data=['Criteria'], barmode='group', title = 'My Academic Research and Activity per Year per Type')

        #print(pub3)

        pubsumm_graph2 = pubsumm_graph2.update_xaxes(categoryorder='category ascending')
