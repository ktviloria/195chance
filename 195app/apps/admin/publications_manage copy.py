...
layout= html.Div( 
    [ 
        dbc.Card(
            [
                dbc.CardHeader(html.H2("Publications Management")),
                dbc.CardBody(
                    [
                        dbc.Row(
                            [
                        ...
                        dbc.Row(
                            [
                                ...
                                dbc.Col(
                                    html.Div(
                                        dcc.Dropdown(id='pubman_date_filter', clearable=True, searchable=True, placeholder="LOWER YYYY"),
                                ...
                                dbc.Col(
                                    html.Div(
                                        dcc.Dropdown(id='pubman_date_filter_upper', clearable=True, searchable=True, placeholder="UPPER YYYY"),
                                        className="dash-bootstrap" 
                                    ),
                                    style={'min-width': '8%', 'padding-left': '0.5rem', 'padding-right': '0.5rem'}
                                ),
                                #search filter
                                dbc.Col(
                                    dbc.Input(type="text", id="pubman_filter", placeholder="Search"),
                                    style={'min-width': '22%', 'padding-left': '0.5rem', 'padding-right': '0.5rem'}
                                ),
                                #active inactive dropdown
                                dbc.Col(
                                    dcc.Dropdown(id='status_filter', clearable=True, searchable=True, placeholder="Status", width = 2),
                                        className="dash-bootstrap" 
                                ), 
                            ],
                            className="mb-3",
    ...
    if pathname == '/publications_manage':
        if tab == 'tab_a':
            sql_a = """SELECT
                ...
                """
                    # CONCAT ('Published in/on ', To_char(a_date, 'Month YYYY'), ' in ', a_pub_name, ' by ', a_publisher),
                    # CONCAT (a_doi, ', ', a_isxn, ', Scopus: ', a_scopus)
            # ORDER BY
            #         authorships.pub_id ASC
            values_a = []
            ...
            sql_o = """SELECT
                ...
                """
            values_o = []
            cols_o = ['Year','id', 'Faculty Involved', 'Publication Criteria', 'Title', 'Date', "Other Info", "Last Updated", "Last Modified By"] 
...
def download_pubman(tab, n_clicks,searchterm, datefilter, datefilter_u):
    ...
    if n_clicks==1:
        ...
        if tab == 'tab_a':
            ...
            download_table_a = dcc.send_data_frame(pub_a_down.to_csv, "authorships_download.csv")
            ...
            download_table_o = dcc.send_data_frame(pub_o_down.to_csv, "others_download.csv")
